...
*/
    ...
    while( mem32required > 0 )
    {
        ...
        uint32_t lines = mem32required / bytes_per_lines;
        size_t segment_size = bytes_per_lines ;
        int mem32alloc = segment_size + sizeof( fb_segment_t ) - sizeof( uint32_t );
        ESP_LOGD( TAG, "mem32alloc %d bytes for framebuffer", mem32alloc );
...
uint8_t* framebuffer_pos( fb_context_t* fbc, uint32_t pos )
{
    ...
    if( ( fb_pos - fbc->fb_offset ) == fbc->fb_segment->size ){
        ...
        ESP_LOGD( TAG, "Next segment  %d - %d = %d -> %d", fb_pos, fbc->fb_offset,fb_pos - fbc->fb_offset, fbc->fb_segment->size );
    }else if( ( fb_pos - fbc->fb_offset ) > ( int )fbc->fb_segment->size ){
        ESP_LOGE( TAG, "Out of sync  %d - %d = %d > %d", fb_pos, fbc->fb_offset,fb_pos - fbc->fb_offset, fbc->fb_segment->size );
    }
